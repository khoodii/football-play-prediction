[final_play_prediction_writeup.md](https://github.com/user-attachments/files/28789521/final_play_prediction_writeup.md)
# Predicting Our Offense's Play Calls from Hand-Logged Football Data

A machine learning and data analysis project built on a full season of our high school team's offensive play-by-play data, recorded by hand in a custom shorthand notation.

## Overview

I set out to answer a single question: can a machine learning model predict our own offense's play calls (run, pass, or QB run) from the game situation — down, distance, and the look the opposing defense gave us (box count and coverage)?

The framing matters: the play calls are *ours*, and the box/coverage fields describe the *defense we lined up against* on each snap. If a model can guess our call from the down-and-distance and the defensive front, then so can an opposing coordinator — which makes this fundamentally a self-scouting question about how predictable our play-calling is.
...
- **Source:** 9 games of our own offensive play-by-play, logged by hand during film review. Each row is a play *we* ran, tagged with the defensive front we faced.
...
3. **Exploratory analysis.** Our run/pass/QB-run rates broken down by down, distance, the defensive box we faced, and opponent.
...
Feature importance confirmed what the exploratory analysis suggested: **down** was the only feature carrying real weight; the defensive box count, coverage, and blitz contributed almost nothing.
...
- **Descriptive tendency tables** — our run/pass rates by situation, which directly serve the self-scouting use case: showing where our own play-calling is predictable enough that a defense could key on it.
...
- **Add features with more variance** — the defensive box count was 62% one value, which limits its predictive power.
